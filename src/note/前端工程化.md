...
![知名企业对 1-3 年经验的前端工程师招聘要求](https://image-host-1257416358.cos.accelerate.myqcloud.com/job-details.png)
...
![Vue.js Logo](https://image-host-1257416358.cos.accelerate.myqcloud.com/logo.png)
...
Vue 组件不仅支持这些语言的所有基础用法，还增加了非常多更高效的功能，在后面 Vue3 教程的一章会有详细的介绍。
...
![image-20221117140852106](https://image-host-1257416358.cos.accelerate.myqcloud.com/image-20221117140852106.png)

右键查看该网页的源代码，可以看到网页内容对应的 HTML 结构也是包含在 `.html` 文件里。

![image-20221117140932149](https://image-host-1257416358.cos.accelerate.myqcloud.com/image-20221117140932149.png)
...
:::tip
Vue Router 默认提供了这两种 URL 改变方式的支持，分别是 `createWebHashHistory` 的 Hash 模式和 `createWebHistory` 对应的 History 模式。
:::
...
![image-20221117141337657](https://image-host-1257416358.cos.accelerate.myqcloud.com/image-20221117141337657.png)
...
![Visual Studio Code 界面截图](https://image-host-1257416358.cos.accelerate.myqcloud.com/screenshot-vscode.jpg)

![Postman 界面截图](https://image-host-1257416358.cos.accelerate.myqcloud.com/screenshot-postman.jpg)
...
![](https://image-host-1257416358.cos.accelerate.myqcloud.com/screenshot-postman-20221117141609947.jpg)
...
- [TypeScript](TyepScript指南.md) 的类型系统和代码检查真好用，也可以放心写，交给构建工具去编译
...
![Webpack 的工作原理（摘自 Vite 官网）](https://image-host-1257416358.cos.accelerate.myqcloud.com/bundler.png)

Vite 是基于浏览器原生的 ES Module ，所以不需要预先打包，而是直接启动开发服务器，请求到对应的模块的时候再进行编译，下面是 Vite 使用的 ESM 机制的工作流程。

![Vite 的工作原理（摘自 Vite 官网）](https://image-host-1257416358.cos.accelerate.myqcloud.com/esm.png)
...
前面在编写 Hello TypeScript 这个 demo 的时候，使用了 `npm run dev:ts` 这样的命令来测试 TypeScript 代码的可运行性，可以把这个阶段认为是的一个 “测试环境” ，这个时候代码不管怎么写，它都是 TypeScript 代码，不是最终要编译出来的 JavaScript 。
...
在 [Hello TypeScript](TyepScript指南.md#hello-typescript) demo 最后配置的一个 `npm run build` 命令，将 TypeScript 代码编译成了 JavaScript ，这个时候 dist 文件夹下的代码文件就处于 “生产环境” 了，因为之后不论源代码怎么修改，都不会直接影响到它们，直到再次执行 build 编译。
