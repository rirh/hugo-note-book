...
在 [前端工程化](工程化指南.md#了解前端工程化) 十分普及的今天，可以说几乎所有要用到的插件，都可以在 [npmjs](https://www.npmjs.com/) 上搜到，除了官方提供的包管理器 npm ，也有很多种安装方式选择。

:::tip
如果还不了解什么是包和包管理器，请先阅读 [了解包和插件](工程化指南.md#了解包和插件) 一节的内容。

另外，每个包管理都可以配置镜像源，提升国内的下载速度，对此也可以先阅读 [配置镜像源](工程化指南.md#配置镜像源) 一节了解。
:::
...
:::tip
如果还没有了解过 “模块” 的概念的话，可以先阅读 [了解模块化设计](工程化指南.md#了解模块化设计) 一节的内容。
:::
...
![注意 npm 包名称后面的 TS 图标](https://image-host-1257416358.cos.accelerate.myqcloud.com/npm-detail-ts-icon.jpg)
...
:::tip
请注意，如果是 Alpha 或者 Beta 版本，通常会在版本号上增加 `-alpha.0` 、 `-alpha.1` 这样的 [版本标识符](工程化指南.md#版本标识符)，以便在发布正式版本的时候可以使用无标识符的相同版本号，以保证版本号在遵循 [升级规则](工程化指南.md#基本格式与升级规则) 下的连续性。
:::